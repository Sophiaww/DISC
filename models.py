...
from typing import Optional, Tuple, Union
import dgl
from modules.loss import RobustBalancedSoftmax
import torch
...
class simpleGNN_MR(nn.Module):
    def __init__(self, in_feats: int, hidden_feats: int, out_feats: int, num_layers: int, e_types: list,
        ...
        self.mlp3_dim = mlp3_dim
        self.bn_type = bn_type
        self.RobustBalancedSoftmax = RobustBalancedSoftmax(2, 0.0)
    ...
    def predict(self, embeddings_np: np.ndarray) -> np.ndarray:
        """
        [HighConfidenceGraphExtractor 的接口]
        接收 NumPy 嵌入，计算 logits，返回 NumPy 预测类别 (0或1)。
        """
        # 1. 转换回 PyTorch Tensor 并移至设备
        device = next(self.parameters()).device
        # dtype 必须匹配 GNN 的特征类型
        h = torch.tensor(embeddings_np, dtype=torch.float32).to(device)

        # 2. 计算 logits (通过 proj_out)
        # 确保 h 是拼接后的特征 (N, H * (L+1))，否则 proj_out 会失败
